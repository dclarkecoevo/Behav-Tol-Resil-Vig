...
IndBehav1<-IndBehav %>%
  ...
  select(-c(Blind.ID, InfLength, LateLength, dayofinf, Countintial, Notes, InfWeight, TrialTime...29, AUC2...35))%>%
  #Rename the other Trial Time column so it aligns with all code and is easier to call for analyses
  rename(TrialTime = TrialTime...22, AUC = AUC2...20)
...
pairs(~AvgVel+TrialTime+Sex+PreWeight+PreLength+Treatment+Infection+LateWeight+AUC+Recov+Died+PreSMI+LateSMI+Wormbf+Ratebf+NRatebf+VarVel+BehavTol+BehavVig+ChBehav+PTol, lower.panel=panel.smooth, diag.panel=panel.hist, 
