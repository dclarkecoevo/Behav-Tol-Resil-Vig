...
IndBehav8$ScPSMI<-as.numeric(IndBehav8$ScPSMI)
...
```{r New Metric of tolerance, warning=FALSE, echo=FALSE}
...
pairs(~AvgVel+Sex+Infection+AUC2+Died+NRatebf+VarVel+BehavTol+BehavVig+TisTol+ResidPLength+Treatment, lower.panel=panel.smooth, diag.panel=panel.hist, 
...
pairs(~AvgVel+VarVel+Sex+Infection+TrialTime+Treatment+ResidPLength, lower.panel=panel.smooth, diag.panel=panel.hist, 
upper.panel=panel.cor, data=IndBehav8)
...
```{r fitting the variance of velocity model, warning=FALSE, include=FALSE}
...
```{r model validation for the Variance in velocity model, warning=FALSE, include=FALSE}
...
```{r visual for variance in velocity, warning=FALSE, include=FALSE}
...
pairs(~ScTisTol+Sex+ScPSMI+ScVarvelBef+ScBehavTol+ScBehavVig+ScRPLength, lower.panel=panel.smooth, diag.panel=panel.hist,
upper.panel=panel.cor, data=IndBehav8)
...
* Deterministic
 + $BehavVig_{det}$ = a + $b_{1}$Sex * $b_{2}$ScPSMI + $b_{3}$ScVarVelBef +
...
#Fit a linear model for checking what explanatory factors are important for Variance in Velocity
#Note this is a linear mixed model because we have multiple measures per fish and therefore, need to account for non-independence between measures. 
BehavVigLM<-glm(BehavVig~Sex*ScPSMI+ScVarvelBef+ScRPLength+Treatment, family=Gamma("log"), IndBehav8)
...
#TrialTimeGraph
BehavVigbySMI=visreg(BehavVigLM, scale="response", "ScPSMI", "Sex", partial=T, gg=TRUE)+
  theme_classic()+
  theme(legend.position="none")+
  ylab("Behavioral Vigor (cm/s)")+
  xlab("Scaled Pre-infeciton mass index")+
  theme(text=element_text(size=22))+
  facet_wrap(~Sex)
#Print the graph
print(BehavVigbySMI)
...
* Deterministic
 + $BehavTol_{det}$ = a + $b_{1}$Sex + $b_{2}$ScRPLength + $b_{3}$ScVarVelBef + $b_{4}$ScPSMI + $b_{5}$ScBehavVig + $b_{5}$ScTisTol
...
#Note this is a linear mixed model because we have multiple measures per fish and therefore, need to account for non-independence between measures. 
BehavTolLM<-lm(BehavTol~Sex*ScRPLength+ScVarvelBef+ScPSMI+ScTisTol+ScBehavVig+Treatment, IndBehavI)
...
```{r visualing the pattern between behavioral tolerance and SMI}
#SMI graph
BehavTolbySMI=visreg(BehavTolLM, scale="response", "ScPSMI", partial=T, gg=TRUE)+
  theme_classic()+
  theme(legend.position="none")+
...
* Deterministic
 + $AUC_{det}$ = a + $b_{1}$Sex * $b_{2}$ScBehavVig + $b_{3}$ScVarVelBef + $b_{4}$ScPSMI + $b_{5}$ScRPLength
...
* Fixed
  ...
  + Visible implant elastomer treatment
  
  
```{r linear model for Infection Intensity}
#Fit a linear model for behavioral tolerance
#Note this is a linear mixed model because we have multiple measures per fish and therefore, need to account for non-independence between measures. 
AUCLM<-glm(AUC2~Sex*ScBehavVig+ScBehavVig+ScVarvelBef+ScPSMI+ScRPLength+Treatment+Sex:ScPSMI, family=Gamma(link="log"), IndBehavI)
...
InfIntbyVarvel=visreg(AUCLM, scale="response", "ScVarvelBef", partial=T, gg=TRUE)+
  theme_classic()+
  theme(legend.position="none")+
  ...
  theme(text=element_text(size=22))
#Print the graph
print(InfIntbyVarvel)
```


```{r visualing the pattern between Infection intensity and Variance in velocity, Warning=FALSE}
#Behavioral Vigor graph
InfIntbyVig=visreg(AUCLM, scale="response", "ScBehavVig", "Sex", partial=T, gg=TRUE)+
  theme_classic()+
  theme(legend.position="none")+
  ylab("Infection intensity")+
...
```{r visualing the pattern between Infection intensity and SMI by sex in velocity, Warning=FALSE}
#Sex by SMI graph
InfIntbyScPSMI=visreg(AUCLM, scale="response", "ScPSMI", "Sex", partial=T, gg=TRUE)+
  theme_classic()+
  theme(legend.position="none")+
  ylab("Infection intensity")+
  ...
  theme(text=element_text(size=22))
#Print the graph
print(InfIntbyScPSMI)
```

```{r visualing the pattern between Infection intensity and Scaled mass index, Warning=FALSE}
#Behavioral Vigor graph
InfIntbySMI=visreg(AUCLM, scale="response", "ScPSMI", partial=T, gg=TRUE)+
  theme_classic()+
  theme(legend.position="none")+
  ylab("Infection intensity")+
  xlab("Scaled body condition")+
...
```{r visualing the pattern between Infection intensity and Sex, Warning=FALSE}
#Behavioral Vigor graph
