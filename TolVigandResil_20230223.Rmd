...
IndBehav3Inf <- IndBehav3 %>%
  ...
  pivot_longer(cols=starts_with("Wormbf"),
               ...
               values_to = "Wormbf")%>%
  select(-c(Velocity_Before, Velocity_Early, Velocity_Late, Velocity_Later))
...
IndBehav3InfV<-IndBehav3 %>%
  ...
  select(-c(Wormbf_Before, Wormbf_Early, Wormbf_Late, Wormbf_Later))
...
IndBehav4<-merge(IndBehav3InfV, IndBehav3Inf1, by.x=c("fishID", "TrialTime"), by.y=c("fishID", "TrialTime"), all=TRUE)%>%
  select(-c(Sex.x, 
            PreWeight.x, 
            PreLength.x, 
            Treatment.x,
            ...
            rate012.y,))%>%
  rename(Sex = Sex.y, 
         PreWeight = PreWeight.y, 
         PreLength = PreLength.y, 
         Treatment = Treatment.y, 
         Infection = Infection.y, 
         LateWeight = LateWeight.y, 
         wormJump = wormJump.y, 
         AUC = AUC.y, 
         Recov = Recov.y, 
         maxworm = maxworm.y, 
         peakday = peakday.y, 
         ratebf_Early = rate05.y, 
         ratebf_Late = rate512.y, 
         ratebf_Later = rate18end.y, 
         PreSMI = PreSMI.y, 
         LateSMI = LateSMI.y, 
         Died = Died.y)
